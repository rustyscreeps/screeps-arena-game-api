use crate::{
    constants::{Direction, Part, ReturnCode, ResourceType},
    prelude::*,
    objects::{ConstructionSite, GameObject, Store, Source, Resource},
...
extern "C" {
    ...
    pub fn move_to(this: &Creep, target: &JsValue, options: Option<&FindPathOptions>) -> ReturnCode;

    /// Take a resource from the ground.
    #[wasm_bindgen(final, method)]
    pub fn pickup(this: &Creep, target: &Resource) -> ReturnCode;

    /// Help another creep to follow this creep. The fatigue generated for the target's move will be added to the creep instead of the target.
    #[wasm_bindgen(final, method)]
...
impl Creep {
    ...
    pub fn transfer<T>(&self, target: &T, ty: ResourceType, amount: Option<u32>) -> ReturnCode where T: Sized + Transferable {
